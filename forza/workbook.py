...
Design goals (the "1000x better than a flat PDF" brief):
  ...
  * Tier-band colour coding by class and confidence-coloured cells.
  * A scannable discipline x class quick-reference matrix.
  * A real Tuning sheet: per-discipline baselines + per-car specs + a
    Share-code column that is honest about provenance.
  * Sources + a Changelog the refresh agent appends to.
"""
...
def _overview_sheet(wb: Workbook, meta: dict) -> None:
    ws = wb.active
    ws.title = "Overview"
...
def _tier_list_sheet(wb: Workbook, cars: list[Car]) -> None:
    ...
    for car in _sorted_cars(cars):
        ...
        for i, v in enumerate(vals, start=1):
            ...
            c.border = BORDER
            c.alignment = WRAP if i in (4, 8, 10, 11, 12, 13) else CENTER
    ...
    _set_widths(ws, {1: 17, 2: 7, 3: 4, 4: 34, 5: 7, 6: 9, 7: 12,
                     8: 30, 9: 13, 10: 46, 11: 38, 12: 30, 13: 22})
    ...
    for disc in DISCIPLINES:
        ...
        for i, cls in enumerate(classes_desc, start=2):
            ...
            if car:
                ...
                cell.font = Font(color=_readable_text(bg), size=9)
        ws.row_dimensions[row].height = 40
        row += 1

    _set_widths(ws, {1: 20, **{i: 22 for i in range(2, 2 + len(classes_desc))}})
